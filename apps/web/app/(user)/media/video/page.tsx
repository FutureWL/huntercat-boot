...
export default function VideoPage() {
  const [url, setUrl] = useState("") // 视频地址输入
  ...
  return (
    <main className="space-y-4">
      <div className="space-y-1">
        <h1 className="text-3xl font-bold tracking-tight">视频 /media/video</h1>
        <p className="text-muted-foreground">先封装原生 video：后续可以加清晰度、倍速、画中画等能力。</p>
      </div>

      <Card className="shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg">播放器</CardTitle>
          <CardDescription>请输入可访问的视频 URL（例如你自己的文件服务或 CDN 地址）。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <div className="flex gap-2">
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => setUrl("https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4")}
              >
                示例
              </Button>
            </div>
          ...
          <div className="rounded-md border bg-background/60 p-3">
            {canPlay ? (
              <video className="aspect-video w-full rounded-md bg-black" controls src={url} />
            ) : (
